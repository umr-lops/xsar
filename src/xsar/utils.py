...
def mask_names(obj):
    ...
    list of str
        mask names
    """
    if obj.__class__.__name__ == 'RadarSat2Dataset':
        meta = obj.rs2meta
    elif obj.__class__.__name__ == 'Sentinel1Dataset':
        meta = obj.s1meta
    return meta._mask_features.keys()


def get_mask(obj, name, describe=False):
    """
    ...
    if obj.__class__.__name__ == 'RadarSat2Dataset':
        ...
        meta = obj.s1meta
    if describe:
        descr = meta._mask_features_raw[name]
        try:
    ...
    if meta._mask_geometry[name] is None:
        poly = _get_mask_intersecting_geometries(obj, name) \
            .unary_union.intersection(meta.footprint)

        if poly.is_empty:
            poly = Polygon()

        meta._mask_geometry[name] = poly
    return meta._mask_geometry[name]


def _get_mask_intersecting_geometries(obj, name):
    if obj.__class__.__name__ == 'RadarSat2Dataset':
        meta = obj.rs2meta
    elif obj.__class__.__name__ == 'Sentinel1Dataset':
        meta = obj.s1meta
    if meta._mask_intersecting_geometries[name] is None:
        gseries = gpd.GeoSeries(_get_mask_feature(obj, name).intersecting_geometries(meta.footprint.bounds))
        if len(gseries) == 0:
            # no intersection with mask, but we want at least one geometry in the serie (an empty one)
            gseries = gpd.GeoSeries([Polygon()])
        meta._mask_intersecting_geometries[name] = gseries
    return meta._mask_intersecting_geometries[name]


def _get_mask_feature(obj, name):
    if obj.__class__.__name__ == 'RadarSat2Dataset':
        meta = obj.rs2meta
    elif obj.__class__.__name__ == 'Sentinel1Dataset':
        meta = obj.s1meta
    # internal method that returns a cartopy feature from a mask name
    if meta._mask_features[name] is None:
        feature = meta._mask_features_raw[name]
        if isinstance(feature, str):
            ...
            footprint_crs = transform(proj_transform, meta.footprint)
        ...
        meta._mask_features[name] = feature

    return meta._mask_features[name]
